...
void Scene3::TextRender()
{
}

void Scene3::ResizeButton()
{
    //타일 시작위치
    if (ImGui::SliderFloat2("B", Tm->B,
        -1000, 1000))
    {
        Tm->InitPosition();
    }
    //타일 한개 사이즈
    if (ImGui::SliderFloat("TileScaleY",
        &Tm->TileScale.y, 0, 500))
    {
        Tm->TileScale.x = Tm->TileScale.y * sqrt3;
        Tm->InitPosition();
    }
    //가로세로 타일 갯수
