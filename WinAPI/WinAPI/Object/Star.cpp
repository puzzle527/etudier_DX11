...
Star::Star(UINT slice, Vector2 pivot)
{
	Slice = slice;

	float Seta = PI / (float)Slice * 2;
	...
	for (UINT i = 0; i < Slice; i++)
	{
		...
		Vertex[i * 3].color = Color(R, G, B, 1.0f);

		Vertex[i * 3 + 1].Position.x = cosf(Seta * 2 * (i)) * 0.5f - pivot.x;
		Vertex[i * 3 + 1].Position.y = sinf(Seta * 2 * (i)) * 0.5f - pivot.y;
		float y = Vertex[i * 3 + 1].Position.y;
		Vertex[i * 3 + 1].color =
			Color(R, G, B, 1.0f);

		Vertex[i * 3 + 2].Position.x = cosf(Seta * 2 * (i + 1)) * 0.5f - pivot.x;
		Vertex[i * 3 + 2].Position.y = sinf(Seta * 2 * (i + 1)) *0.5f - pivot.y;
